import {GraphData, NodeConfig} from '@antv/g6-core/lib/types';
import {ERegisterNodeEnum} from '../hs-graph';

const circleAnimateNode: NodeConfig = {
  id: 'node1',
  x: 100,
  y: 100,
  type: ERegisterNodeEnum.circleAnimate,
  size: 20,
  label: 'circleAnimateNode',
  labelCfg: {
    position: 'top',
  },
};
const backgroundAnimateNode: NodeConfig = {
  id: 'node2',
  x: 300,
  y: 200,
  type: ERegisterNodeEnum.backgroundAnimate,
  color: '#40a9ff',
  size: 20,
  label: 'backgroundAnimateNode',
  labelCfg: {
    position: 'left',
    offset: 10,
  },
};
const innerAnimateNode: NodeConfig = {
  id: 'node3',
  x: 400,
  y: 100,
  size: [40, 40],
  type: ERegisterNodeEnum.innerAnimate,
  img:
    'data:image/webp;base64,',
  label: 'innerAnimateNode',
  labelCfg: {
    position: 'right',
  },
};
const baseNode = {
  id: 'node4',
  x: 300,
  y: 300,
  type: 'rect',
  label: 'baseNode',
  labelCfg: {
    position: 'bottom',
  },
};
export const nodeAnimationDatum: GraphData = {
  nodes: [circleAnimateNode, backgroundAnimateNode, innerAnimateNode, baseNode],
  edges: [
    {
      source: circleAnimateNode.id,
      target: innerAnimateNode.id,
    },
    {
      source: innerAnimateNode.id,
      target: baseNode.id,
    },
    {
      source: baseNode.id,
      target: backgroundAnimateNode.id,
    },
    {
      source: backgroundAnimateNode.id,
      target: circleAnimateNode.id,
    },
  ],
};
